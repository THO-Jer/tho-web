...
export async function POST(req: NextRequest) {
  ...
  try {
    const payload = (await req.json()) as { moduleKey?: string; lessonId?: string; unitSlug?: string; elapsedSeconds?: number; reachedEnd?: boolean; interactionsCompleted?: number };
    ...
    const interactionsCompleted = Number(payload.interactionsCompleted || 0);

    const data = await markLessonCompleted(session.email, moduleKey, lessonId, unitSlug, elapsedSeconds, reachedEnd, interactionsCompleted);
